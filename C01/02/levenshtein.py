...
def leven_dist_input():
    ...
    source_string = input("Enter sooure string: ")
...
def leven_dist_function():
    ...
    for r in range(1, len_source):
        for c in range(1, len_target):
            ...
            d_sub = d_matrix[r - 1][c - 1] + \
                sub_cost(source_string[r], target_string[c])
            d_matrix[r][c] = min(d_del, d_ins, d_sub)

    leven_dist = d_matrix[len_source - 1][len_target - 1]
    print(
        f"Levenshtein distance between {target_string[1:]} and {source_string[1:]} is {leven_dist}")
